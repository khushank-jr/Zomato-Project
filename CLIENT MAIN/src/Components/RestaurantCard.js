...
const RestaurantCard = (props) => {
    ...
    return (
        <Link to={`/restaurant/${props._id}`} className='w-full'>
            <div className='mb-4 bg-white p-4 w-full rounded-2xl transition duration-600 ease-in-out hover:shadow-lg md:w-1/2 lg:w-1/3'>
                <div className='w-full h-56 lg:h-64 relative'>
                    <div className='absolute w-full bottom-4 flex items-end justify-between'>
                        <div className='flex flex-col gap-2 items-start'>
                            ...
                            {props.isOff && (
                                <span className='bg-blue-600 text-white px-2 py-1 rounded  text-xs font-medium'>₹{`${props.isOff}`} OFF</span>
                            )}
                        </div>
                        <span className='bg-white bg-opacity-75 p-1 rounded mr-3 text-xs font-medium'>{props.durationOfDelivery} min</span>
                    </div>
                ...
                <div className='my-2 flex flex-col gap-1'>
                    <div className='flex items-center justify-between'>
                        ...
                        <span className='bg-green-800 text-white p-1 rounded flex items-center text-xs font-medium'>{props.restaurantReviewValue} <AiTwotoneStar /></span>
                    </div>
